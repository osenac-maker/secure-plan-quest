...
const steps = [
  {
    ...
    benefit: "Nous adaptons nos calculs à votre statut et votre tranche d'imposition",
  },
  {
    title: "Votre situation",
    subtitle: "Ces données nous aident à affiner votre projection",
    benefit: "Votre situation familiale impacte directement vos droits et avantages fiscaux",
  },
  {
    title: "Votre épargne",
    subtitle: "Pour calculer vos économies fiscales potentielles",
    benefit: "Nous identifions les leviers pour optimiser votre effort d'épargne",
  },
  {
...
const statusOptions = [
  { value: "freelance", label: "Freelance / Micro-entrepreneur", icon: Briefcase },
...
const SliderField = ({
  ...
  <div>
    <div className="flex items-center justify-between mb-2">
      ...
      <span className="text-sm font-bold text-copper tabular-nums">{format(value)}</span>
    </div>
    <input
...
const formatEur = (v: number) => v >= 1000 ? `${(v / 1000).toFixed(0)} k€` : `${v} €`;
const formatEurFull = (v: number) => v.toLocaleString("fr-FR") + " €";
              ...
              {anticipatedSaving > 0 && (
                ...
                >
                  ...
                  <p className="font-heading text-3xl font-bold text-copper">
                    ...
                    <span className="text-base font-normal text-muted-foreground ml-1">/ an</span>
                  </p>
                </motion.div>
              )}
      ...
      <div className="pt-32 pb-16">
        <div className="container mx-auto px-4 max-w-2xl">
            ...
            {step === 0 && (
              ...
              >
                ...
                <p className="text-muted-foreground text-base max-w-xl mx-auto mb-5">
                  En 2 minutes, obtenez votre projection retraite personnalisée, vos économies
                  fiscales potentielles et un plan d'action concret.
                </p>
                <div className="bg-card border border-border rounded-xl p-4 max-w-lg mx-auto mb-5">
                  <p className="text-sm font-medium text-foreground mb-3">Votre bilan gratuit comprend :</p>
                  <div className="grid grid-cols-3 gap-3 text-xs text-muted-foreground">
                    <span className="flex flex-col items-center gap-1.5 text-center">
                      <TrendingUp className="w-4 h-4 text-copper" />Économies fiscales
                    </span>
                    <span className="flex flex-col items-center gap-1.5 text-center">
                      <Award className="w-4 h-4 text-copper" />Projection retraite
                    </span>
                    <span className="flex flex-col items-center gap-1.5 text-center">
                      <Shield className="w-4 h-4 text-copper" />Protection famille
                    </span>
                  </div>
                </div>
                <div className="flex flex-wrap items-center justify-center gap-4 text-sm text-muted-foreground">
                  <span className="flex items-center gap-1.5"><Clock className="w-4 h-4 text-copper" />2 minutes chrono</span>
                  <span className="flex items-center gap-1.5"><Shield className="w-4 h-4 text-copper" />Sans engagement</span>
                  <span className="flex items-center gap-1.5"><Lock className="w-4 h-4 text-copper" />Données confidentielles</span>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <div className="mb-8">
              ...
              {steps.map((s, i) => (
                <div key={i} className="flex items-center gap-1.5">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300 ${
                    i < step ? "bg-green-500 text-white" : i === step ? "bg-copper text-white shadow-md shadow-copper/30" : "bg-muted text-muted-foreground"
                  }`}>
                    {i < step ? <CheckCircle className="w-4 h-4" /> : i + 1}
                  </div>
                  <span className={`hidden sm:inline text-xs font-medium transition-colors ${i === step ? "text-foreground" : "text-muted-foreground"}`}>
                    {s.title}
                  </span>
                </div>
              ))}
            ...
            <p className="text-xs text-muted-foreground mt-2 text-right">Étape {step + 1} sur {steps.length}</p>
          </div>
            ...
            >
              ...
              {step === 0 && (
                <div className="space-y-6">
                  <div>
                    <Label className="text-foreground mb-2 block">Quel est votre statut professionnel ?</Label>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {statusOptions.map((opt) => {
                        ...
                        return (
                          ...
                          >
                            <Icon className={`w-5 h-5 flex-shrink-0 ${data.status === opt.value ? "text-copper" : "text-muted-foreground"}`} />
                            {opt.label}
                          </button>
                        );
                      })}
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm font-bold text-copper">{data.age} ans</span>
                    </div>
                    <input
                      type="range" min={25} max={65} step={1} value={data.age}
                      onChange={(e) => update("age", Number(e.target.value))}
                      className="w-full h-2 rounded-full appearance-none cursor-pointer accent-copper bg-muted"
                    />
                    <div className="flex justify-between text-xs text-muted-foreground mt-1">
                      <span>25 ans</span><span>65 ans</span>
                    </div>
                  </div>
                  <SliderField
              ...
              {step === 1 && (
                <div className="space-y-6">
                  <div>
                    <Label className="text-foreground mb-2 block">Situation familiale</Label>
                    <div className="grid grid-cols-2 gap-3">
                      {situationOptions.map((s) => (
                        <button
                          key={s.value}
                          onClick={() => update("situationFamiliale", s.value)}
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm font-bold text-copper">{data.enfants}</span>
                    </div>
                    <input
                      type="range" min={0} max={6} step={1} value={data.enfants}
                ...
                <div className="space-y-5">
                  <div className="bg-muted/50 rounded-xl p-4 grid grid-cols-2 gap-3 text-xs mb-2">
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-foreground mt-0.5">{data.age} ans</p>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Revenus annuels</span>
                      <p className="font-semibold text-foreground mt-0.5">{formatEurFull(data.revenu ?? 0)}</p>
                    </div>
                    <div>
              ...
              <div className="flex justify-between items-center mt-8 pt-6 border-t border-border">
                ...
                >
                  {step === steps.length - 1 ? (
                    <><Sparkles className="w-4 h-4" />Découvrir mes résultats</>
                  ) : (
